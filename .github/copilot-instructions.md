...
- **No redundant inline comments**: a comment like `// Re-open after close()` above a clearly named `_openIndexes()` call adds no value. Only add inline comments for non-obvious logic or required context.
...
- **Secondary indexes open on demand**: only the primary index is opened eagerly in `_openIndexes()`. Secondary indexes are opened lazily on first access.
