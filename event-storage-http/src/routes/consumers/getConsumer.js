...
function registerGetConsumerRoute(app, eventStore) {
    app.get('/consumers/:identifier', async (request, response) => {
        ...
        const matchesByIdentifier = consumers.filter(name => name.endsWith(`.${request.params.identifier}`));
        if (matchesByIdentifier.length === 0) {
            throw new HttpError(404, `Consumer "${request.params.identifier}" does not exist.`);
        }
        if (matchesByIdentifier.length > 1) {
            throw new HttpError(409, `Consumer identifier "${request.params.identifier}" is ambiguous.`, matchesByIdentifier);
        }
