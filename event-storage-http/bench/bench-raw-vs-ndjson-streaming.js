...
function createBenchmarkServer(readStore, firstStreamName, streamNames) {
        ...
        const handleRequest = async () => {
            ...
            case '/ndjson-single':
                ...
                for (let pass = 0; pass < passes; pass++) {
                    await streamResponse(response, readStore.getEventStream(firstStreamName, 1, -1, true), pass === passes - 1);
                }
                break;
            case '/ndjson-mixed':
                response.writeHead(200, { 'content-type': 'application/x-ndjson; charset=utf-8' });
                for (let pass = 0; pass < passes; pass++) {
                    await streamResponse(response, readStore.getAllEvents(1, -1, true), pass === passes - 1);
                }
                break;
            default:
...
async function main() {
    ...
    try {
        ...
        const firstStreamName = streams[0];
        const singleStreamDocs = readStore.getStreamVersion(firstStreamName);
        const allDocs = readStore.length;

        const server = createBenchmarkServer(readStore, firstStreamName, streams);
